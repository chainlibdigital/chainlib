...
//        Model::$warehouseName = self::$warehouse->name;
    ...
    {
        $categoriesMenu = ProductCategory::with('products.hoverImage')->where('parent_id', 0)->where('active', 1)->orderBy('position', 'asc')->get();
        $resouresMenu = BlogCategory::where('parent_id', 0)->where('active', 1)->orderBy('position', 'asc')->get();
        View::share('categoriesMenu', $categoriesMenu);
        View::share('resouresMenu', $resouresMenu);
    }

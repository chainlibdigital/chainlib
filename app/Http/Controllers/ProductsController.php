...
class ProductsController extends Controller
{
    ...
    */
    public function productRender(Request $request, $category, $product)
    {
        ...
        return view('front.products.product', compact('seoData', 'category', 'product', 'similarProducts'));
    }
    ...
    public function categoryRender($category)
    {
        ...
        if ($category->children()->count() > 0) {
            ...
            return view('front.products.category', compact('seoData', 'category', 'children'));
        }else{
            $products = Product::with(['translation', 'mainPrice'])->where('category_id', $category->id)->get();
            return view('front.products.subCategory', compact('seoData', 'category', 'products'));
        }
    }
...
//         $data['blogCat'] = [];
        ...
        foreach ($products as $key => $product) {
            $data['products'][$key]['url'] = '/'.$this->lang->lang.'/catalog/'.$product->category->alias.'/'.$product->alias;
            $data['products'][$key]['title'] = $product->translation->name;
            $data['products'][$key]['image'] = $product->image;
        }

        // Find FAQ
        $faqs = FeedBack::where('message', 'like', '%'.$search.'%')
                       ->orWhere('additional_1', 'like', '%'.$search.'%')
                       ->get();

       foreach ($faqs as $key => $faq) {
           $data['faqs'][$key]['url'] = '/'.$this->lang->lang.'/faq';
           $data['faqs'][$key]['title'] = 'FAQ';
       }

        // Find Pages
//        $findPages = PageTranslation::where('title', 'like', '%'.$search.'%')
...
//                        ->where('active', 1)
//                        ->whereIn('id', array_filter($findPages))
//                        ->get();
//
//        foreach ($pages as $key => $page) {
//            $data['pages'][$key]['url'] = '/'.$this->lang->lang.'/'.$page->alias;
//            $data['pages'][$key]['title'] = $page->translation->title;
//        }

        // Find Blogs
//        $findBlogs = BlogTranslation::where('name', 'like', '%'.$search.'%')
//                                   ->orWhere('body', 'like', '%'.$search.'%')
//                                   ->get()
//                                   ->pluck('blog_id')
//                                   ->toArray();
...
//                    ->where('active', 1)
//                    ->whereIn('id', array_filter($findBlogs))
//                    ->get();
//
//        foreach ($blogs as $key => $blog) {
//            $data['blogs'][$key]['url'] = '/'.$this->lang->lang.'/resources/list/'.$blog->category->alias;
//            $data['blogs'][$key]['title'] = $blog->category->translation->name;
//        }

        // Find Blogs Cats
//        $findBlogsCats = BlogCategoryTranslation::where('name', 'like', '%'.$search.'%')
//                                   ->orWhere('description', 'like', '%'.$search.'%')
//                                   ->get()
//                                   ->pluck('blog_category_id')
//                                   ->toArray();
//
//        $blogsCats = BlogCategory::with(['translation'])
//                    ->where('active', 1)
//                    ->whereIn('id', array_filter($findBlogsCats))
//                    ->get();

//        foreach ($blogsCats as $key => $blogsCat) {
//            $data['blogCat'][$key]['url'] = '/'.$this->lang->lang.'/resources/'.$blogsCat->alias;
//            $data['blogCat'][$key]['title'] = $blogsCat->translation->name;
//        }

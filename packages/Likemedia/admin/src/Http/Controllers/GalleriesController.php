...
class GalleriesController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->get('new_iframe')) {
            foreach ($request->get('new_iframe') as $key => $newIframe) {
                GalleryImage::where('id', $key)->update([
                    'src' => $newIframe
                ]);
            }
        }
        ...
        if ($request->file('new_video')) {
            foreach ($request->file('new_video') as $key => $newVideo) {
                $videoName = uniqid().$newVideo->getClientOriginalName();
                $path = public_path().'/images/galleries/videos/';
        ...
        if ($request->file('new_image')) {
            foreach ($request->file('new_image') as $key => $newImage) {
                $hash = uniqid();
                $imageName = $hash.$newImage->getClientOriginalName();
                $image_resize = Image::make($newImage->getRealPath());
                $image_resize->save(public_path('images/galleries/og/' .$imageName), 75);
                $type = "image";
                $imageName = '/images/galleries/og/' . $imageName;
        ...
        if (request('name')) {
            foreach (request('name') as $key => $values) {
                foreach ($values as $langId => $value) {
                    GalleryImageTranslation::where('gallery_image_id', $key)->where('lang_id', $langId)->update([
                        'text' => request('text')[$key][$langId],
                        'title' => request('name')[$key][$langId],
                        'link' => request('link')[$key][$langId],
                        'alt' => request('alt')[$key][$langId],
                    ]);
                }
            }
        }

...
use App\Models\Lang;
use App\Models\CategoriesOpacId;
...
class ProductCategoryController extends Controller
{
    ...
    public function addNewId(Request $request)
    {
        $opacId = CategoriesOpacId::where('opac_id', $request->get('id'))->first();

        $opacItem = CategoriesOpacId::create([
            'opac_id' => $request->get('id'),
            'category_id' => $request->get('category_id'),
        ]);

        $data['fields'] = $this->getOpacItem($opacItem);
        $data['categories'] = $this->getCategories();
    ...
    {
        $productField = $request->product;
        $product = Product::findOrFail($productField['productId']);

        if ($productField['title']) {
            $alias = str_slug($productField['title']);
        }else{
            $alias = uniqid(10);
        }
        ...
        $product->update([
            'alias' => $alias,
            'issn' => $productField['issn'],
        ]);

        foreach ($this->langs as $key => $langItem) {
            $product->translations()->where('lang_id', $langItem->id)->update([
                'name' => $productField['title'],
                'author' => $productField['author'],
                'co_author' => $productField['co_author'],
                'language' => $productField['language'],
                'subject' => $productField['subject'],
                'publication' => $productField['publication'],
            ]);
        }
    ...
    public function getOpacItem($opacItem)
    {

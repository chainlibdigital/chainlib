...
class PagesController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $this->addPhotosVideos($request, $page);
    ...
    public function addPhotosVideos($request, $page)
    {
        ...
        if($files = $request->file('photos')){
            foreach($files as $key => $file){
                ...
                $image_resize->save(public_path('images/pages/og/' .$name), 75);

                $image_resize->resize(960, null, function ($constraint) {
                                $constraint->aspectRatio();
                            })->save('images/pages/md/' .$name, 85);

                $image_resize->resize(480, null, function ($constraint) {
                                $constraint->aspectRatio();
                            })->save('images/pages/sm/' .$name, 85);
    ...
    public function update(Request $request, $id)
    {
        ...
        $bannerRight = $request->banner_right_old;
        ...
        if ($request->banner_right) {
            $bannerRight = uniqid() . '-' . $request->banner_right->getClientOriginalName();
            $request->banner_right->move('images/pages', $bannerRight);
            if ($page->image_right) {
                @unlink(public_path('images/pages/'.$page->image_right));
            }
        }
